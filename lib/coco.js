 ...
 */
Coco.prototype.verifyNotify = function(remoteip, reqdata, callback) {
    ...
    if (verify) {
        callback(true, '{"status":"ok"}', order);
    } else {
        callback(false, '{"status":"failure"}', order);
    }
};
